...
// src/internals.browser.ts
var internals_browser_exports = {};
__export(internals_browser_exports, {
  BrowserPixelLoader: () => BrowserPixelLoader,
  MmcqQuantizer: () => MmcqQuantizer,
  WasmQuantizer: () => WasmQuantizer,
  classifySwatches: () => classifySwatches,
  ...
  paletteOklchScaledToRgb: () => paletteOklchScaledToRgb,
  pixelsRgbToOklchScaled: () => pixelsRgbToOklchScaled,
...
module.exports = __toCommonJS(internals_browser_exports);
...
// src/quantizers/wasm.ts
var WasmQuantizer = class {
  /**
   ...
   */
  constructor(module2, wasmUrl) {
    this.wasmQuantize = null;
    this.module = module2;
  ...
  async init() {
    ...
    if (!this.module) {
      throw new Error(
        'WasmQuantizer requires the wasm-bindgen module built from src/wasm. Build it with `npx wasm-pack build src/wasm --target web`, then pass the generated glue module: `new WasmQuantizer(await import("./pkg/color_thief_wasm.js"))`.'
      );
    }
    try {
      if (typeof this.module.default === "function") {
        await this.module.default(
          this.wasmUrl ? { module_or_path: this.wasmUrl } : void 0
        );
      }
  ...
  quantize(pixels, maxColors) {
    ...
    for (let offset = 0; offset + 6 < resultBytes.length; offset += 7) {
      ...
      const population = view.getUint32(offset + 3, true);
...
var REMOVED = "the Web Worker path was removed in v3 and these shims will be deleted in v4. Run Color Thief inside your own worker instead \u2014 see the README.";
function isWorkerSupported() {
  deprecate(`isWorkerSupported() always returns false \u2014 ${REMOVED}`);
  return false;
}
function extractInWorker(pixels, maxColors, signal, nativeGamut = "srgb", outputGamut = "srgb") {
    ...
    try {
      ...
      resolve(
        raw.map(({ color, population }) => {
          const [r, g, b] = nativeGamut === outputGamut ? color : p3ToSrgb(color[0], color[1], color[2]);
          return createColor(
            r,
            g,
            b,
...
//# sourceMappingURL=internals.browser.cjs.map